...
export const GetPitchforkTrackList = (startDateStr?: string, endDateStr?: string): Promise<MusicTrack[]> => {
    ...
    const FetchTracklist = async (startLocation: number = 0): Promise<void> => {
        try {
    ...
    return new Promise<MusicTrack[]>((resolve, reject) => {
        if (_.isNil(startDateStr)) {
            ...
            endDate = _.isNil(endDateStr)
                ? startDate
                      .clone()
                      .add(1, 'weeks')
                      .subtract(1, 'day')
                : moment(endDateStr, DATE_FORMAT);
        }
        ...
        Promise.all(promises)
            .then(() => {
                let sortedAndFilteredTrackListData: List[] = _.chain(tracklistData)
                    .filter(track => {
